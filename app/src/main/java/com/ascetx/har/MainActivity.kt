...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    companion object {
        ...
        private val mx= mutableListOf<Float>()
        private val my= mutableListOf<Float>()
        private val mz= mutableListOf<Float>()
    }
...
//            && mx.size >= TIME_STAMP && my.size >= TIME_STAMP && mz.size >= TIME_STAMP
        ) {
...
//            data.addAll(mx.subList(0, TIME_STAMP))
//            data.addAll(my.subList(0, TIME_STAMP))
//            data.addAll(mz.subList(0, TIME_STAMP))

            results = classifier.predictProbabilities(toFloatArray(data))
            Log.i(TAG, "ax: $ax")
            Log.i(TAG, "predictActivity: ${results.contentToString()}")
            //'walking', 'standing', 'jogging', 'sitting', 'biking', 'upstairs',
            //       'downstairs'
            bikingTextView.text = "Biking: \t${round(results[4], 2)}"
            downstairsTextView.text = "DownStairs: \t${round(results[6], 2)}"
            joggingTextView.text = "Jogging: \t${round(results[2], 2)}"
            sittingTextView.text = "Sitting: \t${round(results[3], 2)}"
            standingTextView.text = "Standing: \t${round(results[1], 2)}"
            upstairsTextView.text = "Upstairs: \t${round(results[5], 2)}"
            walkingTextView.text = "Walking: \t${round(results[0], 2)}"
